    ...
    fn try_from(req: Request<'a>) -> Result<Bytes, Self::Error> {
        ...
        match req {
            ReadBits(address, quantity, code) | ReadWords(address, quantity, code) => {
                request_command(&mut data, address, code, quantity);
            }
            WriteMultipleBits(address, bits, code) => {
                request_command(&mut data, address, code, bits.len().try_into().unwrap());
                // data.put_slice(&bit_to_bytes(&bits));
            ...
            WriteMultipleWords(address, words, code) => {
                request_command(&mut data, address, code, words.len().try_into().unwrap());
...
impl TryFrom<(Bytes, Request<'_>)> for Response {
    ...
    fn try_from((bytes, req): (Bytes, Request)) -> Result<Self, Self::Error> {
        ...
        match req {
            ...
            Request::ReadWords(_, quantity, _) => {
            ...
            Request::WriteMultipleBits(_, _, _) => Ok(Response::WriteMultipleBits),
            Request::WriteMultipleWords(_, _, _) => Ok(Response::WriteMultipleWords),
        }
    }
}
...
fn request_byte_count(req: &Request<'_>, header_len: usize) -> usize {
    use crate::frame::Request::*;
    match *req {
        ReadBits(_, _, _) | ReadWords(_, _, _) => header_len + REQUEST_BYTE_LAST_LEN,
        WriteMultipleBits(_, ref bits, _) => {
...
fn request_command(data: &mut BytesMut, address: u32, code: SoftElementCode, cnt: u16) {
...
mod tests {
    ...
    fn test_read_words_to_bytes() {
        ...
        let expected_bytes = vec![
            ...
            0x00, // 0x0C 为请求数据的长度
            0x10, 0x00, //, 0x00, 0x01,
            0x01, 0x04, 0x00, 0x00, 0x21, 0x43, 0x65, 0xA8, 0x14, 0x00,
        ];
